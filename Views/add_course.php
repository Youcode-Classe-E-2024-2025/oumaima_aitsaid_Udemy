...
<div id="videoFields" class="hidden">
    <div class="mb-4">
        ...
        <input type="text" name="video_title" id="video_title" placeholder="Enter video title" required
            class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
    </div>
    <div class="mb-4">
        <label class="block text-sm font-medium text-gray-700 mb-1">Upload Video</label>
        <input type="file" name="video_resource" id="video_resource" accept="video/*" required
            class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
    </div>
</div>
...
<div id="documentFields" class="hidden">
    <div class="mb-4">
        ...
        <input type="text" name="document_title" id="document_title" placeholder="Enter document title" required
            class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
    </div>
    <label for="markdown-editor" class="block text-sm font-medium text-gray-700 mb-1">Markdown Content</label>
    <textarea 
        id="markdown-editor" 
        name="content_text" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    resourceType.addEventListener('change', function() {
        const videoFields = document.querySelectorAll('#videoFields input');
        const documentFields = document.querySelectorAll('#documentFields input, #documentFields textarea');
        
        [...videoFields, ...documentFields].forEach(field => {
        ...
        if (this.value === 'video') {
            ...
            documentFields.forEach(field => field.required = true);
        }
    });
